...
const AllArticle = () => {
  ...
  const userPremiumTakenMap = Object.fromEntries(
    users.map((user) => [user._id, user.premiumTaken])
  );

  // Function to check if the user is subscribed based on premiumTaken
  const isUserSubscribed = (userId) => {
    const premiumTaken = userPremiumTakenMap[userId];
                  ...
                  <Link to={`/article/${article._id}`}>
                    <button
                      className={`inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 ${
                        article.isPremium && !isUserSubscribed(article.authorId)
